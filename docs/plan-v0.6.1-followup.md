...
- 修复 AppShell 导航可达性，避免“评估结果、模板库、设置”等入口长期只显示 disabled 状态而没有规划说明。
...
目标：新增 `/templates`，提供本地模板库页面，帮助用户复用简历段落、投递说明、面试准备和风险解释表达。

关键变化：
- 提供模板分类：简历亮点、项目经历、投递说明、面试准备、风险解释、证据补充提醒。
- 模板先以本地静态数据或轻量文件形式存在，不引入远程模板服务。
- 模板详情展示适用场景、可替换变量、示例文本、证据约束和禁用场景。
- 支持按分类、关键词、风险等级和适用阶段筛选。
- 模板使用必须强调“不能编造硬事实”，与 v0.5.7 的 `safe_rewrites`、`simulated_supplements`、`forbidden_gaps` 语义一致。

验收标准：
- 页面可从 AppShell 导航进入。
- 模板空态、分类筛选、详情展开、复制文本入口有测试覆盖。
- 模板文案不承诺自动投递，不要求保存敏感信息。
...
- API key、完整 CV/JD 原文和敏感路径不应出现在设置页、日志摘要或模板页面中。
